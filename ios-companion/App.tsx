...
export default function App() {
  ...
  const [newTaskAccountableDirectReportId, setNewTaskAccountableDirectReportId] = useState<number | null>(null);
  ...
  const tasks = useMemo(() => dashboard ? orderTasks(dashboard.tasks, dashboard.categories).filter((task) => (category === "all" || task.categoryId === category) && (completed || !task.done) && taskMatchesPriority(task, priority) && (responsibleColleagueFilter === "all" || (responsibleColleagueFilter === "unassigned" ? task.accountableDirectReportId === null : task.accountableDirectReportId === responsibleColleagueFilter))) : [], [dashboard, category, completed, priority, responsibleColleagueFilter]);
  ...
  const closeSheet = () => { setSheet(null); setText(""); setEditingCategory(null); setEditingReport(null); setSubCategoryParent(null); setSelectedDestinationCategoryId(null); setSelectedParentTaskId(null); setNewCategoryName(""); setNewParentTaskName(""); setCreatingNewCategory(false); setCreatingNewParent(false); setDestinationPickerOrigin(null); setReorderParent(null); setReorderCategoryId(null); setMovingTask(null); setNewTaskPriority("medium"); setNewTaskDueAt(null); setNewTaskRecurrence("none"); setNewTaskAccountableDirectReportId(null); setShowDueDatePicker(false); setEmailTo(""); setEmailCc(""); setEmailSubject(""); setEmailBody(""); setEmailReview(false); };
  const openSheet = (next: Exclude<Sheet, null>) => { haptic(); setText(""); setNewCategoryName(""); setNewParentTaskName(""); setCreatingNewCategory(false); setCreatingNewParent(false); setDestinationPickerOrigin(null); setNewTaskPriority("medium"); setNewTaskDueAt(null); setNewTaskRecurrence("none"); setNewTaskAccountableDirectReportId(null); setShowDueDatePicker(false); setEmailTo(""); setEmailCc(""); setEmailSubject(""); setEmailBody(""); setEmailReview(false); setSheet(next); };
  ...
  const createTask = async (parent: Task | null) => {
    ...
    const input: TaskCreateInput = { categoryId, ...(parentId ? { parentId } : {}), text: text.trim(), sortOrder: siblingCount, dueAt: newTaskDueAt, priority: newTaskPriority, recurrence: newTaskRecurrence, accountableDirectReportId: newTaskAccountableDirectReportId, mobileClientMutationId: `mobile-create-${Date.now()}-${Math.random().toString(36).slice(2)}` };
    ...
    <Modal visible={sheet !== null} transparent animationType="slide" onRequestClose={closeSheet}><View style={styles.backdrop}><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={8} style={styles.keyboardAvoider}>{sheet === "manageCategories" ? <CategoryManagerSheet categories={dashboard?.categories ?? []} height={managementSheetHeight} onMove={moveCategory} onEdit={editCategory} onDelete={deleteCategory} onDone={closeSheet} /> : <ScrollView ref={isTaskFormSheet ? taskFormScrollRef : undefined} style={[styles.modalScroll, isTaskFormSheet && styles.taskSetupScroll]} contentContainerStyle={[styles.modalScrollContent, isTaskFormSheet && styles.taskSetupScrollContent]} scrollEnabled={sheet !== "selectCategory" && sheet !== "selectParent" && sheet !== "selectLandingCategory"} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" showsVerticalScrollIndicator={isTaskFormSheet}><View style={[styles.modal, isTaskFormSheet && styles.taskSetupModal]}>
      ...
      {sheet === "category" || sheet === "directReport" || sheet === "task" || sheet === "subCategory" ? <><Text style={styles.modalTitle}>{formTitle}</Text>
        ...
        {(sheet === "task" || sheet === "subCategory") && <View style={styles.taskSetup}>
          ...
          <View style={styles.taskSetupSection}><Text style={styles.label}>Responsible Colleague</Text><View style={styles.detailChips}><Pressable accessibilityRole="button" accessibilityLabel="Assign no Responsible Colleague to new task" onPress={() => setNewTaskAccountableDirectReportId(null)} style={[styles.chip, newTaskAccountableDirectReportId === null && styles.chipOn]}><Text style={styles.chipText}>N/A</Text></Pressable>{(dashboard?.directReports ?? []).map((report) => <Pressable key={report.id} accessibilityRole="button" accessibilityLabel={`Assign Responsible Colleague ${report.name} to new task`} onPress={() => setNewTaskAccountableDirectReportId(report.id)} style={[styles.chip, newTaskAccountableDirectReportId === report.id && styles.chipOn]}><Text style={styles.chipText}>{report.name}</Text></Pressable>)}</View></View>
