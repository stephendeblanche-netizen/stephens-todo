...
export default function App() {
  ...
  return <GestureHandlerRootView style={styles.gestureRoot}><SafeAreaView style={styles.screen}><StatusBar style="dark" />
    ...
    <Modal visible={sheet !== null} transparent animationType="slide" onRequestClose={closeSheet}><View style={styles.backdrop}><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={8} style={styles.keyboardAvoider}>{sheet === "manageCategories" ? <CategoryManagerSheet categories={dashboard?.categories ?? []} height={managementSheetHeight} onMove={moveCategory} onEdit={editCategory} onDelete={deleteCategory} onDone={closeSheet} /> : <ScrollView style={[styles.modalScroll, (sheet === "task" || sheet === "subCategory") && styles.taskSetupScroll]} contentContainerStyle={[styles.modalScrollContent, (sheet === "task" || sheet === "subCategory") && styles.taskSetupScrollContent]} scrollEnabled={sheet !== "selectCategory" && sheet !== "selectParent" && sheet !== "selectLandingCategory"} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" showsVerticalScrollIndicator={sheet === "task" || sheet === "subCategory"}><View style={[styles.modal, (sheet === "task" || sheet === "subCategory") && styles.taskSetupModal]}>
...
const styles: Record<string, any> = new Proxy(StyleSheet.create({
  ...
  taskSetupScroll: { flexGrow: 0, flexShrink: 1, maxHeight: "92%" },
  taskSetupScrollContent: { flexGrow: 1, justifyContent: "flex-end", paddingTop: 24 },
  taskSetupModal: { gap: 14, paddingBottom: 34 },
